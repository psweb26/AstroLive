...
import type { AstrologerRecord, Recommendation } from '@/src/core/recommendation/types';

export function AstrologerProfile({ id, directoryAstrologer }: { id: string; directoryAstrologer?: AstrologerRecord }) {
  ...
  if (!recommendation) {
    ...
    return <ProductShell><main className="product-page page-frame"><section className="mx-auto max-w-3xl py-12 sm:py-20"><p className="orbital-mark ml-3 text-xs font-semibold uppercase tracking-[0.18em] text-signal-secondary">Specialist introduction</p><h1 className="mt-5 font-display text-5xl font-semibold leading-[0.94] tracking-[-0.04em] text-ink sm:text-6xl">{directoryAstrologer.name}</h1><p className="mt-5 max-w-2xl text-xl leading-8 text-ink-secondary">{directoryAstrologer.specializations.join(' · ')}</p><p className="mt-4 max-w-2xl text-base leading-7 text-ink-secondary">{directoryAstrologer.short_description}</p><dl className="mt-12 grid gap-6 border-y border-line py-7 sm:grid-cols-3"><div><dt className="text-xs font-semibold uppercase tracking-[0.14em] text-ink-muted">Experience</dt><dd className="mt-2 text-lg font-semibold text-ink">{directoryAstrologer.experience_years} years</dd></div><div><dt className="text-xs font-semibold uppercase tracking-[0.14em] text-ink-muted">Languages</dt><dd className="mt-2 text-lg font-semibold text-ink">{directoryAstrologer.languages.join(', ')}</dd></div><div><dt className="text-xs font-semibold uppercase tracking-[0.14em] text-ink-muted">Consultation range</dt><dd className="mt-2 text-lg font-semibold text-ink">₹{directoryAstrologer.price_min}–₹{directoryAstrologer.price_max}</dd></div></dl><div className="mt-10 border-l-2 border-signal-secondary pl-5"><p className="font-display text-2xl leading-[1.25] text-ink sm:text-3xl">Begin with your question so AstroLive can prepare a relevant specialist and consultation context.</p><Button asChild variant="signal" className="mt-7"><Link href="/understanding-you">Start an insight</Link></Button></div></section></main></ProductShell>;
  }
    ...
    <ProductShell>
      <main className="product-page page-frame">
        ...
        <section className="border-t border-line pt-10 lg:grid lg:grid-cols-[minmax(0,1fr)_18rem] lg:gap-20">
          <div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-signal-secondary">Consultation</p><h2 className="mt-4 text-3xl font-semibold text-ink sm:text-4xl">Continue with a focused conversation.</h2><p className="mt-4 max-w-2xl text-lg leading-8 text-ink-secondary">{astrologer.short_description}</p><p className="mt-5 text-sm text-ink-muted">{astrologer.methods.join(' · ')} · ₹{astrologer.price_min}–₹{astrologer.price_max}</p></div>
          <div className="mt-7 lg:mt-0"><Button asChild variant="signal"><Link href={`/booking?astrologer=${astrologer.id}`}>Choose a consultation time</Link></Button><p className="mt-4 text-sm leading-6 text-ink-muted">AstroLive will carry this recommendation into the selected prototype time.</p></div>
        </section>
      </main>
    </ProductShell>
  );
